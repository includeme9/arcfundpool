...
export default function ContributorPage() {
  ...
  return (
    <section className="app-container py-8 md:py-12">
      ...
      <div className="scrollbar-hide mt-6 flex gap-3 overflow-x-auto pb-2 md:grid md:grid-cols-3">
            ...
            return (
              <div key={item.id} className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 md:flex md:items-center md:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-[var(--muted)]">{formatUSDC(item.amount)} · {formatDate(item.timestamp)} · {shortenAddress(item.txHash, 8)}</p>
                </div>
                <button disabled={!canRefund} className="tap-target mt-4 rounded-full bg-cyan-300 px-5 py-2 text-sm font-semibold text-slate-950 disabled:opacity-45 md:mt-0">
                  Claim refund
                </button>
              </div>
            );
